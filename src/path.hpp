...
std::string canonicalFromRelativePath(const std::string &location,
                                      const Path &path) {
...
template <typename Path>
std::string
canonicalFromAbsolutePath(const std::string &location,
...
handlePath(const std::string &location, const Path &path,
           const std::string &server_url, FindCDNURL findCDNUrl) {
  // First get the canonical version of the string
  std::string canonical(
      utils::is_relative(path.begin(), path.end())
          ? canonicalFromRelativePath(location, path)
          : canonicalFromAbsolutePath(location, path,
                                      server_url));
