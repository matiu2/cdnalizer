/**
 * Performs general tests on rewriteHTMl, just checking the input and output
 *strings.
...
go_bandit([]() {
  ...
  DataEvent newData = [&](std::string data) { output.append(data); };
  ...
  describe("High level Rewrite HTML", [&]() {
    it("1. Handles double quote attributes", [&]() {
      ...
      AssertThat(
          output,
          Is().EqualTo(R"**(<img src="http://cdn.supa.ws/imgs/a.gif">)**"));
    });
    ...
    it("3. Rewrites full server urls correctly", [&]() {
